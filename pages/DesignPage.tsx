// ─── Design Page — Orquestrador principal da Aba Design ─────────────────────
//
// Fluxo:
...
import { WizardTabs } from '@/components/design/WizardTabs';
import { WizardEstilo } from '@/components/design/WizardEstilo';
import { WizardFoto } from '@/components/design/WizardFoto';
import { WizardSite } from '@/components/design/WizardSite';
import { WizardPapelaria } from '@/components/design/WizardPapelaria';
import { SitePreview } from '@/components/design/SitePreview';
import { SaveTheDatePreview } from '@/components/design/SaveTheDatePreview';
    ...
    <div className="absolute bottom-0 left-0 right-0 z-10 flex justify-around border-t border-black/5 bg-white pb-6 pt-2">
      {tabs.map((tab) => (
        ...
        >
          ...
          <span
            className="font-['Josefin_Sans'] text-[8.5px] font-light tracking-[0.13em] uppercase"
            style={{ color: tab.active ? accent : '#3D322A' }}
          >
            {tab.label}
          </span>
        </div>
...
function Header({
  ...
  return (
    <div
      className="sticky top-0 z-[5] flex items-center gap-2 border-b border-black/5 bg-[#FAF6EF]/95 px-5 py-2.5 backdrop-blur-sm"
    ...
    >
      ...
      <p
        className="flex-1 text-center font-['Josefin_Sans'] text-[12px] font-light tracking-[0.28em] uppercase"
        style={{ color: '#3D322A' }}
      >
        {title}
      </p>
...
export default function DesignPage() {
  ...
  const openWizard = useCallback((step: WizardStep = 'estilo') => {
    setWizardStep(step);
    goTo('wizard');
  }, [goTo]);
  ...
  return (
    // Phone shell — simulates a mobile screen frame
    <div className="flex min-h-screen items-start justify-center bg-[#E8E0D4] px-4 py-10">
      <div
        className="relative overflow-hidden rounded-[44px] border-[8px] border-[#1e1a16] bg-[#FAF6EF] shadow-2xl"
        style={{
          width: 390,
          minHeight: 844,
      ...
      >
        ...
        {/* Status bar */}
        <div className="flex items-center justify-between px-7 pb-1 pt-[14px] text-[11px] font-medium text-[#3D322A]">
          <span>9:41</span>
        ...
        {/* ── Wizard tabs (only when in wizard screen) ── */}
        <AnimatePresence>
          {screen === 'wizard' && (
            <motion.div
        ...
        <div className="overflow-y-auto" style={{ maxHeight: 'calc(844px - 44px - 80px)' }}>
          <AnimatePresence mode="wait">
            ...
            {/* HOME */}
            {screen === 'home' && (
              <motion.div key="home" initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -24 }} transition={{ duration: 0.22 }}>
                <DesignHome
                  toolkit={toolkit}
            ...
            {screen === 'wizard' && wizardStep === 'estilo' && (
              <motion.div key="estilo" initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -24 }} transition={{ duration: 0.22 }}>
                <WizardEstilo
            ...
            {screen === 'wizard' && wizardStep === 'foto' && (
              <motion.div key="foto" initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -24 }} transition={{ duration: 0.22 }}>
                <WizardFoto
            ...
            {screen === 'wizard' && wizardStep === 'site' && (
              <motion.div key="site" initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -24 }} transition={{ duration: 0.22 }}>
                <WizardSite
                  toolkit={toolkit}
            ...
            {screen === 'wizard' && wizardStep === 'papelaria' && (
              <motion.div key="papelaria" initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -24 }} transition={{ duration: 0.22 }}>
                <WizardPapelaria
                  toolkit={toolkit}
            ...
            {/* PREVIEW SITE */}
            {screen === 'preview-site' && (
              <motion.div key="preview-site" initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -24 }} transition={{ duration: 0.22 }}>
                <SitePreview
                  toolkit={toolkit}
                  onBack={() => openWizard('site')}
                />
              </motion.div>
            )}
            ...
            {screen === 'preview-std' && (
              <motion.div key="preview-std" initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -24 }} transition={{ duration: 0.22 }}>
                <SaveTheDatePreview
                  toolkit={toolkit}
