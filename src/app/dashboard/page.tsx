      ...
      {hasPartner ? (
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            {!confirmUnlink && (
              <button
                ...
                disabled={unlinking}
                className="text-xs font-body text-midnight/30 hover:text-red-400 transition"
              >
                Desvincular
              </button>
            )}
          ...
          {confirmUnlink && (
            <div className="flex items-center gap-2 bg-red-50 rounded-xl px-3 py-2">
              <p className="font-body text-xs text-red-700 flex-1">Remover acesso do parceiro?</p>
              <button onClick={unlinkPartner} disabled={unlinking}
                className="px-2.5 py-1 rounded-lg text-xs font-body font-medium bg-red-400 text-white hover:bg-red-500 transition">
                {unlinking ? "…" : "Remover"}
              </button>
              <button onClick={() => setConfirmUnlink(false)}
                className="px-2.5 py-1 rounded-lg text-xs font-body text-red-600 hover:bg-red-100 transition">
                Não
              </button>
            </div>
          )}
        ...
        <div className="space-y-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-xs font-body text-green-600 hover:text-green-700 transition"
          >
                      ...
                      confirmDeleteId === w.id ? (
                        <div className="ml-auto flex items-center gap-1.5 flex-shrink-0">
                          <button onClick={() => deleteWedding(w.id)} disabled={deletingId === w.id}
                            className="px-2 py-0.5 rounded-lg text-[10px] font-body font-medium bg-red-500 text-white hover:bg-red-600 transition">
                            Apagar
                          </button>
                          <button onClick={() => setConfirmDeleteId(null)}
                            className="px-2 py-0.5 rounded-lg text-[10px] font-body text-white/70 hover:bg-white/10 transition">
                            Não
                          </button>
                        </div>
                      ) : (
                          ...
                          confirmDeleteId === ww.id ? (
                            <div className="flex items-center gap-1">
                              <button onClick={() => deleteWedding(ww.id)} disabled={deletingId === ww.id}
                                className="px-2 py-1 rounded-lg text-xs font-body font-medium bg-red-500 text-white hover:bg-red-600 transition">
                                Apagar
                              </button>
                              <button onClick={() => setConfirmDeleteId(null)}
                            ...
                            <button
                              onClick={() => setConfirmDeleteId(ww.id)}
