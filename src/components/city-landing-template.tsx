...
export default function CityLandingTemplate({
  ...
  return (
    <div className="min-h-screen bg-fog font-body text-midnight">
      {/* Header */}
      <header className="bg-midnight text-white">
        <div className="max-w-5xl mx-auto px-4 py-4 flex items-center justify-between">
          <Link href="/" className="font-display text-2xl text-white">Laço</Link>
          <Link
            href="/registro"
            className="px-4 py-2 bg-gold text-white rounded-lg text-sm font-medium hover:bg-gold/90 transition"
          >
            Criar conta grátis
          </Link>
        </div>
      ...
      {/* Hero */}
      <section className="bg-midnight text-white pb-16 pt-12 px-4">
        <div className="max-w-3xl mx-auto text-center">
          <p className="font-body text-gold text-sm uppercase tracking-widest mb-3">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Link
              href="/registro"
              className="px-8 py-3 bg-gold text-white rounded-xl font-body font-semibold hover:bg-gold/90 transition"
            >
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {[
              ...
              <div key={f.title} className="bg-white rounded-2xl shadow-sm p-6">
                ...
                <p className="font-body text-sm text-midnight/60 leading-relaxed">{f.text}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <ul className="space-y-3">
            {highlights.map((h, i) => (
              <li key={i} className="flex items-start gap-3">
                <span className="w-5 h-5 bg-midnight/20 text-midnight rounded-full flex items-center justify-center text-xs mt-0.5 shrink-0">✓</span>
                <p className="font-body text-midnight/70">{h}</p>
              </li>
            ))}
          </ul>

          <div className="mt-8 bg-fog rounded-2xl p-6">
            <p className="font-body text-sm text-midnight/50 mb-1">Custo médio de casamento</p>
            <p className="font-heading text-2xl text-midnight">{avgCost}</p>
      ...
      {/* CTA */}
      <section className="py-16 px-4 bg-midnight text-white text-center">
        ...
        <Link
          href="/registro"
          className="inline-block bg-gold text-white font-body font-semibold px-10 py-4 rounded-xl hover:bg-gold/90 transition text-lg"
        >
